...
@section('content')
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Danh sách quốc gia</h1>
        <div class="btn-toolbar mb-2 mb-md-0">
            <div class="btn-group me-2">
                <a href="{{ route('admin.countries.create') }}" class="btn btn-sm btn-outline-secondary">
            ...
            <tbody>
                @foreach ($countries as $index => $country)
                    <tr>
                        <td>{{ $index + 1 }}</td>
                        <td>{{ $country['title'] }} </td>
                        <td>{{ $country['description'] ?? '[N/A]' }} </td>
                        <td>{{ $country['created_at']->format('d/m/Y') }} </td>
                        <td class="d-flex gap-3">
                            <a href="{{ route('admin.countries.edit', $country['id']) }}" class="text-decoration-none">
